...
// namespace — a hub that links into the moderation queue, the
// decline-patterns view (per-cause), and the cross-cause identity-
// clustering view. PRD §Curator console: read-only workspace; the
...
<main>
  ...
  <p>Read-only workspace. Actions (accept, reject, defer, revoke, archive, expire) run through the curator's MCP agent (Claude Desktop, Cursor, custom client) firing the <code>curator_*</code> tools — the console surfaces what's queued or flagged.</p>
  ...
  <h2>Decline patterns</h2>
  ${renderPerCauseDeclineLinks(directory)}
...
function renderPerCauseDeclineLinks(directory: CauseDirectory): Raw {
  if (directory.causes.length === 0) {
    return emptyState(
      'No active causes — the decline-pattern projection is cause-scoped, so it has nothing to project against yet.',
    );
  }
  return html`<ul>
${directory.causes.map(
