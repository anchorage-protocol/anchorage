...
A first parameter sweep is wired as the Phase-1-exit-criterion infrastructure pattern: a 2D matrix over (coalition decorrelation rate ∈ {0.5, 1.0}, anti-correlation threshold ∈ {0, 0.5, 1.0}) drives six it.each cells, each running the full scenario and asserting the contested-status outcome. The matrix shows where the defense kicks in: threshold 0 leaves the bypass open at any rate; threshold 0.5 fires at boundary equality (≥) for both rates; threshold 1.0 closes only the perfect-decorrelation case, not the mixed one. The sweep harness is currently inline (a SweepCell array + it.each); production sweeps will run more cells and aggregate attack-success-rate, but the v0 surface locks in the per-cell outcome as an invariant readable from the test names alone.

Next milestones: a multi-dimensional cluster signal (calibration-record contrast or payload-feature vote patterns) that closes the mixed-strategy gap; broader sweeps (more knobs, more cells, aggregated attack-success-rate); the remaining archetypes (patient adversary, sybil farms, coalitions across multiple proposals). Live-fetch verification (PMID/DOI/URL resolution and source caching) stays a stub until the testbed needs it; span verification is wired at the verifier seam and exercised by adversary scenarios.
