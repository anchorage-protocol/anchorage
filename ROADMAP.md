...
Next milestones: more attack patterns and knob axes feeding the same aggregate-ASR shape (the remaining seams the composition-closes-mixed-strategy result names — coalitions that avoid co-voting on contentious items, coalitions across multiple proposals); claim-difficulty-normalized review credit (PRD §Reputation) so the gate thresholds are calibrated against effort rather than count. Identity-cost / rate-limit / anti-abuse defenses for the sybil-amplified-coalition seam live below the wired surface and are the next architectural layer the design needs, not a testbed knob. Live-fetch verification (PMID/DOI/URL resolution and source caching) stays a stub until the testbed needs it; span verification is wired at the verifier seam and exercised by adversary scenarios.
