...
The decline-pattern → assignment-gate is now wired as the first defense knob for that seam. PRD §Capacity and assignment commits the gate as the assignment-time form of the abuse-signal handling the existing line names; mechanics mirror the rep gates seam-for-seam — `assignment_max_decline_rate` (default 1.0, gate inert) and `assignment_decline_min_offers` (small-sample floor) consumed at `request_assignment`, refusal as `not_found` so the contributor-facing surface stays structurally indistinguishable from "no work available," and the contributor-initiated path (`cast_review_vote` without `assignment_id`) bypasses by construction so a contributor whose decline rate has spiked retains the recovery path declines are non-punitive precisely *to* preserve. The gate's mechanics are pinned standalone: above-threshold + above min_offers refuses; above-threshold but below min_offers bypasses; fresh reviewer (zero offers) bypasses; the contributor-initiated path bypasses. The numerator and denominator the gate consumes match the curator-side `declinePatterns` projection byte-for-byte — same per-cause cumulative rate — so the assignment-time and curator-side surfaces operate on a single signal. Closure on the multi-proposal seam itself is throughput-mediated rather than retroactive: the gate doesn't undo a lone-reject that already landed, it caps the rate at which the coalition can place new ones; the orthogonal-defenses ASR cube against the cluster signal (cluster closes co-voting coalitions; decline-gate closes paired-decline coalitions; neither alone closes both) lands in the next cycle.

Next milestones: the orthogonal-defenses ASR cube around the new decline-pattern gate — cells over (coalition shape ∈ {co-voting, paired-decline}, cluster-signal stack ∈ {off, on}, decline-gate ∈ {off, on}), with the headline that each defense closes its own coalition shape and the composition closes both, mirroring the existing two cubes' template; claim-difficulty-normalized review credit (PRD §Reputation) so the gate thresholds are calibrated against effort rather than count — a known approximation of the current uniform-credit model that the patient-adversary gate thresholds are tuned against, and load-bearing because the priming-via-easy-items signature is exactly what the patient archetype exploits when credit is uniform. Identity-cost / rate-limit / anti-abuse defenses for the sybil-amplified-coalition seam live below the wired surface and are the next architectural layer the design needs, not a testbed knob. Live-fetch verification (PMID/DOI/URL resolution and source caching) stays a stub until the testbed needs it; span verification is wired at the verifier seam and exercised by adversary scenarios.
