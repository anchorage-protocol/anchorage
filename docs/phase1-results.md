...
The headline reading: **redundant peer review with calibration-aware convergence + stratified-by-history assignment + the four-layer sybil-resistance composition (Binding cost / rate-limited Tickets / cluster-signal Networks / Encounter-domain extension) closes every coalition pattern the testbed has constructed against it on the scripted tier**, with the model-backed cube confirming the closures hold against real-LLM honest pools and adversaries on the deep loop's small-population fixture. Two recorded model-backed closure failures on a borderline-contested item under the bare v0 closure stack motivated three additional `ReviewConfig` knobs (`escalation_revise_counts_as_reject`, `escalation_requires_votes_to_accept`, `contested_votes_to_accept`), which form a v3 stack pinned byte-for-byte at the harness level by scripted-decider pairs in [`population-loop.test.ts`](../packages/server/src/population-loop.test.ts) and exercised at the real-model level by dedicated cube cells. **No open closure failure remains in the cube as of this snapshot.**
...
**6. The model-backed deep-loop cube confirmed the scripted closures hold against real-LLM populations and surfaced two closure failures on a borderline-contested item, both closed.** The strategic adversary handed a genuinely borderline contested claim (source uses observational "associated with" framing; pre-staged claim swaps it for causal "increases") plausibly defends it. Three sampling rollouts of that scenario (`borderline-contested` + `borderline-contested-r2` + `borderline-contested-r3`) show 2/3 fail the bare v0 closure stack via two distinct paths: escalation accept-on-tie (1-1-1 tally with a careful "revise" splitting the rejecting side) and auto-close-accept (≥2 accepts hitting `votes_to_accept`). Both failure paths are closed by the v3 stack (`escalation_revise_counts_as_reject` + `escalation_requires_votes_to_accept` + `contested_votes_to_accept`), pinned byte-for-byte at the harness level. The contested-item-severity axis maps a drift-pattern landscape: verb-swap drift breaks v0 (2/3 rollouts), surrogate-substitution drift v0 closes via curator escalation reject, generalization-from-subgroup drift v0 closes via auto-close-reject — two of three drift kinds the v0 stack handles.
...
- **Sampling rollouts for `borderline-surrogate` and `borderline-subgroup`** — the verb-swap trio established that single-rollout cube readings can be sampling-unlucky; analogous trios on the other drift kinds would tighten the landscape.
