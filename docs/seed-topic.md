...
- **Real disagreement across many sub-areas.** Hull machinery (multi-parent synthesis, parallel positions, `open_question` nodes) earns its keep where the field actually disagrees, not where it has settled.
...
**Microbiome signatures predictive of CRC risk.** A Hull-shaped graph is uniquely valuable here — the field has clarity problems, and a graph that makes the actual evidence chain explicit would separate established from speculative claims. *But* the corpus noise is exactly what would stress v0 governance hardest. Defer until the regime has been exercised on cleaner sub-topics first.
