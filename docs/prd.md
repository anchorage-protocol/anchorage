...
- **Issuance-frequency cap.** Per-(IdP, IP, ASN, …) per-epoch caps on the rate at which new identities can be minted. The cap lives below the MCP layer — the server does not mint identities and has no surface to enforce this directly. PRD commits the layer's existence; specific cap shapes are operationally private at the IdP. The cap is the *time* primitive on the cost-multiplier: an adversary affording K sybils still cannot mint them all in one epoch, which buys behavior-based defenses (cluster signal, calibration record, reputation gates) the accumulated history they need to fire. *Implementation status:* spec is settled here; the testbed harness models the cap as a per-epoch issuance budget on the adversary (see [Adversary testbed](#adversary-testbed)'s adversary-budget model).
