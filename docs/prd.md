...
Vote-history remains a behavioral signal an adversary can game: a coalition can vote independently (or deliberately disagree) on non-bias-zone proposals so its pairwise agreement falls below the cluster threshold, holding distinct strata while still co-voting on bias-zone slots. Contention weighting does not close that vector — a pair that disagrees on every shared proposal still has weighted agreement zero, regardless of contention — and the testbed confirms it stands open. Closing it requires a different refinement: mixing in calibration-item agreement (where ground truth is known), building strata from payload-feature vote patterns rather than raw co-occurrence, or detecting *anti*-correlation as a separate signal. Which refinement does the work is the next testbed target on this seam.
