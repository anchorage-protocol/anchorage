...
- **Bounded identities-per-real-person.** Identity creation has a non-trivial cost — email verification at minimum, third-party OIDC (GitHub, ORCID, institutional SSO) preferred. The cost is tunable. Zero-cost identities are not supported. *Implementation status:* the binding-cost surface lives below the v0 MCP layer (the server treats identities as already-issued and authenticated; minting is the layer below) and no testbed knob expresses it today. The sybil-amplified-coalition scenario — where Eve mints fresh and walks past every behavior-based defense by construction, since cluster signal, calibration record, and reputation all require accumulated per-identity history — is the regression handle that names the gap. The eventual sweep dimension is coalition-affordable-identities-per-epoch (the cost-multiplier on the population axis, composing with the per-identity gates by multiplying their cost across K sybils); specifying this layer is the next architectural milestone (see ROADMAP §Status).
...
- **Sybil farm.** One operator running many identities at the per-identity cost set in the [identity model](#identity). The load-bearing defenses (identity-binding cost, rate-limited issuance, global anti-abuse signals) live below the v0 MCP layer and are not testbed-knob-expressible today — see [Identity](#identity) for the wiring status. What the testbed *does* exercise is the complementary shape — the sybil-amplified-coalition scenario, where a fresh recruit walks past every behavior-based defense by construction, since cluster signal, calibration record, and reputation gates all require accumulated per-identity history. The MCP-layer closure stack composes with the identity layer below; it does not substitute for it.
