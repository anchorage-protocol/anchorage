...
- **Review-credit normalized by claim difficulty.** Without normalization, the regime selects for reviewers who accept easy proposals. Difficulty proxies — review effort, prior divergence, sub-topic frontier-distance — weight review-credit so that engaging hard syntheses is not dominated by rubber-stamping easy ones. *Implementation status:* v0 runs uniform-credit (every accurate review credits the same amount, independent of difficulty) as a known approximation. The patient-adversary signature — priming through easy unanimous accepts on well-grounded excerpts to harvest rep cheaply — is exactly the pattern uniform-credit blesses, so the assignment-gate thresholds (`assignment_min_recent`, `assignment_min_demonstrated`) are tuned against the uniform-credit assumption today and re-baseline when difficulty-normalization lands. The contention signal already computed for the cluster-stratification refinement (vote variance per proposal) is the natural difficulty proxy for the first iteration: easy unanimous-accept items contribute zero contention weight to the cluster signal and would contribute reduced credit weight to the rep ledger by the same primitive.
