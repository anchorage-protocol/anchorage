...
- **Per-identity rate-limit accounting.** Per-(identity, action-kind, epoch) counters on observable write actions (`propose_*`, `cast_review_vote`, `request_assignment`). When the counter hits the configured cap the tool refuses with a new `rate_limited` mode parallel to `not_found` — the contributor sees throttling, not "no work available," because the two have different recovery paths and conflating them would mislead. The cap is the per-sybil-throughput axis on the cost-multiplier: at K sybils with per-sybil cap T, the coalition's per-epoch action budget is bounded by K × T. The signal is *global per identity* — the same identity's actions across two causes both consume the same per-action-kind cap — for the same reason cross-cause anti-abuse is asymmetric to per-cause reputation: a sybil farm working two causes is more detectable than one working one, and the cost of opacity is small relative to the defense. *Implementation status:* spec is settled here; contracts and server wiring land in the rate-limit slice (see ROADMAP §Status).
