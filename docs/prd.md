...
- **Bounded identities-per-real-person via binding cost.** Identity creation has a non-trivial cost — email verification at minimum, third-party OIDC (GitHub, ORCID, institutional SSO) preferred. The cost is tunable per IdP. Zero-cost identities are not supported. Each issued identity carries a numeric `attestation_level` recorded by the IdP at mint, opaque to the server (the server gates on threshold; it does not interpret what the level *means* — that's the IdP's job). The MCP-layer gate is `min_attestation_level`, consumed at `request_assignment`, all `propose_*` tools, and `cast_review_vote`, refusing with the same `not_found` mode the rep gates use. *Implementation status:* spec is settled here; contracts and server wiring land in the binding-cost slice (see ROADMAP §Status).
