...
Anti-correlation is a separate cluster cue alongside agreement: a pair that systematically votes the *opposite* way on shared proposals is also displaying coordinated behavior — just inversely. Treating perfect anti-correlation (1.0 disagreement ratio) as a co-stratum edge catches the decorrelating-coalition variant cleanly, since a coalition that defeats the agreement metric by voting opposite on every shared priming proposal lights up the disagreement metric instead. The testbed scenario for the decorrelating coalition closes against this defense at threshold 1.0; the threshold is testbed-tuned, with weaker thresholds catching noisier adversaries at the cost of admitting honest reviewers who happen to systematically disagree as false positives. The seam that remains open is the *gap* between the two thresholds: a coalition that mixes some agreement with some disagreement on non-bias-zone proposals, sitting below the agreement threshold and below the anti-correlation threshold, looks neither correlated nor anti-correlated to v0. The testbed wires that mixed-strategy coalition end-to-end and confirms it bypasses the regime — a 50/50 vote pattern (two agreed, two disagreed across four shared proposals) evades both thresholds at 1.0 and the bias-suppression vector reopens. Closing that gap requires a signal that doesn't reduce to a single agreement/disagreement ratio: calibration-item-specific agreement (where ground truth is known and a coalition's bias on real items is contrasted against its calibration record), or payload-feature vote patterns (clustering on what a pair votes about, not just whom they agree with). Which refinement does the work is the next testbed target on this seam.
