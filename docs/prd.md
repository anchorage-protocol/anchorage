...
The v0 stratum primitive is the **vote-pattern co-occurrence cluster**: two reviewers fall in the same stratum when their pairwise vote agreement on shared past proposals exceeds a threshold over a rolling window. Coalition members coordinating on bias-zone proposals concentrate into a single cluster by their own behavior; honest reviewers spread across clusters by their independence. New reviewers (no shared history yet) sit each in their own singleton cluster, contributing maximally to diversity by default. Window sizes, agreement thresholds, and cross-stratum draw weights are testbed-tuned; the *signal* — vote-history clustering as the v0 stratum — is committed. Vote-history is a behavioral signal an adversary can game: a coalition can vote independently (or deliberately disagree) on non-bias-zone proposals so its pairwise agreement falls below the cluster threshold, holding distinct strata while still co-voting on bias-zone slots. That adaptation is wired in the [adversary testbed](#adversary-testbed) and confirms what the v0 cluster primitive cannot catch on its own; closing it requires a richer cluster signal — weighting shared proposals by contention so unanimous-easy votes count for less than split-contentious ones, mixing in calibration-item agreement, or building strata from payload-feature vote patterns rather than raw co-occurrence. Which refinement does the work is the next testbed target on this seam.
