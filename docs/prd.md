...
- **Cross-cause identity-clustering projection.** Curator-side projection (`server.curator.identityClusters({ window_epochs, min_signal })`) parallel to the existing `declinePatterns` projection, surfacing identity pairs whose behavioral fingerprint across causes suggests sybil coordination — co-creation timestamps, vote-time correlation across causes, co-occurrence on contested items in unrelated sub-topics. The signal is *global per identity*, with documented governance and audit. Specific signal shapes and thresholds are operationally private (the same posture as `declinePatterns`'s small-sample floor: methodology is public, tuning is not). *Implementation status:* spec is settled here; contracts and server wiring land in the clustering slice (see ROADMAP §Status).
