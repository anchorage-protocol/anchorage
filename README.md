...
Phase 2 slice 1 (live-fetch verification) closed 2026-05-14: `LiveFetchVerifier` resolves PMIDs against NCBI E-utilities and DOIs against Crossref, with shared normalization (smart quotes, dashes, ellipsis, whitespace) so a contributor's quoted span matches the fetched source even when the publisher's typography drifts from ASCII. The production-runtime wiring (slice 4) comes later in the phase; testbed scenarios continue to use `FakeVerifier` with seeded source maps.
