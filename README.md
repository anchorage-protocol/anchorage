...
19 parameter-sweep cubes (18 scripted, PRD-enumerated #1–#18, + 1 model-backed) aggregate attack-success-rate per defense config — and, where a defense's failure mode is collapsing the honest pool rather than letting the attack through, lockout-rate alongside it. Three closure-stack knobs (`escalation_revise_counts_as_reject`, `escalation_requires_votes_to_accept`, `contested_votes_to_accept`) closed the model-backed cube's two recorded closure failures on a borderline contested item; no open closure failure remains in the cube as of this snapshot.

Phase 2 slices 1–7 closed between 2026-05-14 and 2026-05-15. The code surface is in place: a `LiveFetchVerifier` resolving PMIDs against NCBI E-utilities and DOIs against Crossref with shared smart-quote-and-dash normalization (slice 1); a SQLite-backed `Store` on Node's built-in `node:sqlite` with a parity test pinning byte-identical state against the in-memory testbed default (slice 2); GitHub OAuth as the v1 IdP behind an `Authenticator` seam, with agent-as-delegate bearer tokens (slice 3); an HTTP transport, the curator role, the `anchorage-admin` CLI, and a `run-prod.ts` runtime entrypoint with a Dockerfile (slice 4); six MCP read-path resources (`cause://`, `sub-topic://`, `node://`, `subgraph://`, `contributor://`, `manuscript://`) and a server-rendered HTML web read-UI in `packages/web` consuming them in-process (slices 5 and 6); a v0 manuscript projection emitting four fixed-order sections (sources / quotations / synthesis / open_questions) with credit-ordered contributor attribution (slice 6); and a curator-only MCP tool surface, read-only curator console at `/curator/*`, and re-verification scheduler that flips drifted anchors to terminal `unresolvable` (slice 7). On top of that a co-hosted MCP-spec OAuth 2.1 authorization server landed (2026-05-16): any MCP client discovers it, registers, and authorizes through GitHub with no hand-run curl and no copied bearer — the add-and-go path the *60 seconds to deploy* section promises.

**The instance is live.** Anchorage is deployed and serving — [anchorage.science](https://anchorage.science) for the read UI, `mcp.anchorage.science` for the write-path MCP server — with the seed cause and its first sub-topic seated and the OAuth write path open to contributors. What remains for the Phase 2 exit criterion is slice 8 — the first testbed-caught governance change: an operational milestone, not a code slice, met when at least one governance change is caught by the testbed and revised before shipping during live operation. Testbed scenarios continue to use `FakeVerifier` and the in-memory `Store`; the sim≡prod invariant holds across every slice, the live deployment included.
